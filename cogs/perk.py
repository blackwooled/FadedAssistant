...
class Perk(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        self.weekly_perk_task.start()
    
    def cog_unload(self):
        self.weekly_perk_task.cancel()

    @tasks.loop(hours=24)
    async def weekly_perk_task(self):
        ...
        if now.hour != 0 or now.minute != 0:
            next_midnight = (now + timedelta(days=7)).replace(hour=0, minute=0, second=0, microsecond=0)
    ...
    @weekly_perk_task.before_loop
    async def before_weekly_perk_task(self):
    ...
    @commands.command(name="assigncrowns", help="Manually assign crowns to users based on their perks.", hidden=True)
    @is_admin()
